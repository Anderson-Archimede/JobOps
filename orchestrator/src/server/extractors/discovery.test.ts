...
describe("extractor discovery", () => {
  ...
  it("loads all discovered manifests without mock (validates @shared resolution)", async () => {
    const paths = await discoverManifestPaths();
    ...
    for (const manifestPath of paths) {
      ...
      expect(Array.isArray(manifest.providesSources)).toBe(true);
