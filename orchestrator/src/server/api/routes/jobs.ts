...
async function executeJobActionForJob(
  ...
  try {
    ...
    if (action === "move_to_ready") {
      ...
      // Enqueue scoring job instead of synchronous processing
      const queueJob = await scoringQueue.add("score-job", {
        jobId,
        forceRescore: options?.forceMoveToReady ?? false,
      });

      logger.info(`[Jobs Route] Job ${jobId} enqueued for scoring`, {
        queueJobId: queueJob.id,
      });

      // Return immediately with queue job info (cast as any for async result)
      return {
        jobId,
        ok: true,
        queued: true,
        ...
        statusUrl: `/api/queues/scoring/jobs/${queueJob.id}`,
      } as any;
    }
    ...
    // Enqueue scoring job instead of direct scoring
    const queueJob = await scoringQueue.add("score-job", {
      jobId: job.id,
      forceRescore: true,
    });

    logger.info(`[Jobs Route] Job ${job.id} enqueued for rescoring`, {
      queueJobId: queueJob.id,
    });
...
jobsRouter.post("/:id/generate-pdf", async (req: Request, res: Response) => {
  try {
    ...
    // Enqueue tailoring job instead of synchronous PDF generation
    const queueJob = await tailoringQueue.add("tailor-cv", {
      jobId: req.params.id,
      regenerate: false,
    });

    logger.info(`[Jobs Route] Job ${req.params.id} enqueued for PDF generation`, {
      queueJobId: queueJob.id,
    });

    // Return immediately with queue job info
    return res.json({
      success: true,
      queued: true,
      queueJobId: queueJob.id,
      message: "PDF generation queued",
      statusUrl: `/api/queues/tailoring/jobs/${queueJob.id}`,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    res.status(500).json({ success: false, error: message });
