...
async function validateLlm(options: {
  ...
  const fromEnvKey = process.env.LLM_API_KEY?.trim() || null;
  const fromEnvProvider = process.env.LLM_PROVIDER?.trim() || undefined;

  let resolvedApiKey =
    options.apiKey?.trim() || storedApiKey?.trim() || fromEnvKey;
  let normalizedProvider =
    options.provider?.trim() || storedProvider?.trim() || fromEnvProvider;

  if (resolvedApiKey === fromEnvKey && fromEnvProvider) {
    normalizedProvider = fromEnvProvider;
  } else if (!normalizedProvider && fromEnvProvider) {
    normalizedProvider = fromEnvProvider;
  }

  normalizedProvider = normalizedProvider || undefined;
