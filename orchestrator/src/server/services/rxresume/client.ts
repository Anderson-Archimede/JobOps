...
export class RxResumeClient {
  ...
  static async verifyCredentials(
    ...
    try {
      // Reactive Resume v4: email is case-sensitive; normalize to lowercase (see RR issue #1740).
      const normalizedIdentifier =
        identifier.includes("@")
          ? identifier.trim().toLowerCase()
          : identifier.trim();
      const body: { identifier: string; password: string; email?: string } = {
        identifier: normalizedIdentifier,
        password,
      };
      if (normalizedIdentifier.includes("@")) body.email = normalizedIdentifier;
